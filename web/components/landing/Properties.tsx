 ...
 */
const SIDE = "tw:relative tw:z-10 tw:m-0 tw:max-w-xs tw:px-10 tw:text-sm tw:text-neutral-600";

function Cell({ n, title, hard, how, star, index }: Property & { index: number }) {
  const top = index < 3;
  const cls = [
    "tw:group tw:relative tw:flex tw:flex-col tw:border-neutral-200 tw:py-10 tw:lg:border-r",
    ...
    <div className={cls} data-reveal style={{ ["--d" as string]: `${index * 0.06}s` }}>
      <div
        ...
        className={
          ...
          (top ? "tw:bg-linear-to-t tw:from-neutral-100 tw:to-transparent" : "tw:bg-linear-to-b tw:from-neutral-100 tw:to-transparent")
        }
      />
      <div className="tw:relative tw:z-10 tw:mb-4 tw:flex tw:items-center tw:gap-2 tw:px-10 tw:font-mono tw:text-sm tw:text-neutral-500">
        <span>{String(n).padStart(2, "0")}</span>
        {star && (
          <span className="tw:rounded-full tw:border tw:border-neutral-300 tw:px-2 tw:py-0.5 tw:text-[10px] tw:uppercase tw:tracking-widest tw:text-neutral-600">
            hard to get elsewhere
          </span>
        )}
      </div>
      <h3 className="tw:relative tw:z-10 tw:mb-3 tw:px-10 tw:text-lg tw:font-bold tw:tracking-normal tw:text-black">
        <span aria-hidden="true" className="tw:absolute tw:inset-y-0 tw:left-0 tw:h-6 tw:w-1 tw:rounded-tr-full tw:rounded-br-full tw:bg-neutral-300 tw:transition tw:duration-200 tw:group-hover:bg-black" />
        <span className="tw:inline-block tw:transition tw:duration-200 tw:group-hover:translate-x-2">{title}</span>
      </h3>
      <p className={SIDE}><b className="tw:font-semibold tw:text-black">What normally goes wrong:</b> {hard}</p>
      <p className={`${SIDE} tw:mt-3`}><b className="tw:font-semibold tw:text-black">Here:</b> {how}</p>
    </div>
  );
}
