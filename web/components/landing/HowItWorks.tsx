...
const TITLE = "tw:m-0 tw:text-left tw:text-xl tw:font-medium tw:tracking-tight tw:text-black tw:dark:text-white tw:md:text-2xl tw:md:leading-snug";
const DESC = "tw:m-0 tw:my-2 tw:max-w-sm tw:text-left tw:text-sm tw:text-neutral-600 tw:dark:text-neutral-300 tw:md:text-base";
    ...
    <ol className="tw:m-0 tw:mt-6 tw:flex tw:list-none tw:flex-col tw:gap-0 tw:p-0" aria-label="The order of operations">
      {steps.map(([t, d], i) => (
        <li key={t} className="tw:relative tw:flex tw:gap-3 tw:pb-5 tw:pl-1 tw:last:pb-0">
          {i < steps.length - 1 && <span aria-hidden="true" className="tw:absolute tw:top-4 tw:bottom-0 tw:left-[0.9rem] tw:w-px tw:bg-neutral-200 tw:dark:bg-neutral-700" />}
          <span aria-hidden="true" className={"tw:relative tw:z-10 tw:mt-1 tw:flex tw:h-6 tw:w-6 tw:flex-none tw:items-center tw:justify-center tw:rounded-full tw:border tw:bg-white tw:dark:bg-black tw:font-mono tw:text-[10px] " + (i === 1 ? "tw:border-black tw:dark:border-white tw:text-black tw:dark:text-white" : "tw:border-neutral-300 tw:dark:border-neutral-600 tw:text-neutral-500 tw:dark:text-neutral-400")}>{i + 1}</span>
          <span className="tw:flex tw:flex-col">
            <span className={"tw:font-mono tw:text-sm " + (i === 1 ? "tw:font-semibold tw:text-black tw:dark:text-white" : "tw:text-neutral-800 tw:dark:text-neutral-200")}>{t}</span>
            <span className="tw:text-xs tw:text-neutral-500 tw:dark:text-neutral-400">{d}</span>
          </span>
        </li>
      ))}
    </ol>
...
function Record() {
  ...
  return (
    <dl className="tw:m-0 tw:mt-6 tw:overflow-hidden tw:rounded-xl tw:border tw:border-neutral-200 tw:dark:border-neutral-700 tw:bg-white tw:dark:bg-black tw:font-mono tw:text-xs tw:shadow-aceternity">
      {rows.map(([k, v, on]) => (
        <div key={k} className="tw:flex tw:items-baseline tw:justify-between tw:gap-4 tw:border-b tw:border-neutral-100 tw:dark:border-neutral-800 tw:px-4 tw:py-2.5 tw:last:border-0">
          <dt className={on ? "tw:text-black tw:dark:text-white" : "tw:text-neutral-400 tw:dark:text-neutral-500 tw:line-through"}>{k}</dt>
          <dd className={"tw:m-0 tw:text-right " + (on ? "tw:text-neutral-600 tw:dark:text-neutral-300" : "tw:text-neutral-400 tw:dark:text-neutral-500")}>{v}</dd>
        </div>
      ))}
    </dl>
  );
    ...
    <section id="how" data-anchor className="tw:relative tw:z-20 tw:overflow-hidden tw:p-0 tw:py-10 tw:lg:py-40" aria-labelledby="how-h">
      ...
      <div className="tw:relative">
        <div className="tw:mt-12 tw:grid tw:grid-cols-1 tw:lg:grid-cols-6">
          <div className={`${CARD} tw:border-b tw:border-neutral-200 tw:dark:border-neutral-700 tw:lg:col-span-4 tw:lg:border-r`} data-reveal>
      ...
      <p data-reveal className="tw:mt-8 tw:flex tw:flex-wrap tw:justify-center tw:gap-x-8 tw:gap-y-2 tw:text-center tw:text-sm tw:text-neutral-600 tw:dark:text-neutral-300">
